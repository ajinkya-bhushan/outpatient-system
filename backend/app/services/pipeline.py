...
def run_from_transcript(
    ...
    *,
    ...
    record = store.get_or_create(encounter_id, source=source)
    extraction = extract_entities(transcript.text, encounter_id=record.id)
    soap = create_soap(extraction.entities, encounter_id=record.id, user_inputs=user_inputs)
    logger.info("pipeline_completed", encounter_id=record.id, entity_count=extraction.entity_count)
    return PipelineResponse(
        encounter_id=record.id,
        transcript=transcript,
        entity_count=extraction.entity_count,
        category_counts=extraction.category_counts,
